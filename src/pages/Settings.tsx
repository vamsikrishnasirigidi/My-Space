...
export const SettingsPage: React.FC = () => {
  const { user, isGuest, updateProfile, signOut, changePassword, loading } = useAuthStore();
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (!/[A-Z]/.test(newPassword) || !/[0-9]/.test(newPassword)) {
      toast.error('Use at least 1 uppercase letter and 1 number.');
      return;
    }
    ...
    try {
      ...
      useActivityStore.getState().addLog('system', 'Updated account password');
      setIsPasswordModalOpen(false);
      resetPasswordForm();
    } catch {
      // Toast handling is done by store.
    } finally {
    ...
    <div className="flex flex-col gap-6 animate-fade-in max-w-4xl select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <div className="glass-panel rounded-3xl p-5 bg-white dark:bg-slate-900/30 flex flex-col items-center text-center gap-4">
          ...
          <div className="flex flex-col gap-2 w-full">
            <div className="flex items-center justify-between text-xxs font-bold text-slate-400 dark:text-slate-500 uppercase px-1">
              ...
              <span className="text-slate-700 dark:text-slate-300">
                {isGuest ? 'Guest Session' : 'Supabase Auth'}
              </span>
            </div>
            <div className="flex items-center justify-between text-xxs font-bold text-slate-400 dark:text-slate-500 uppercase px-1">
              <span>Joined</span>
          ...
          <button
            onClick={() => {
              if (isGuest) {
                toast.info('Guest users do not have passwords. Upgrade account to enable this.');
                return;
              }
              setIsPasswordModalOpen(true);
            }}
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-indigo-200 dark:border-indigo-900/40 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 font-bold text-xs transition-all cursor-pointer"
          >
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md rounded-3xl border border-slate-200/50 bg-white p-6 shadow-premium dark:border-slate-800/40 dark:bg-slate-900">
            ...
            <form onSubmit={handleChangePassword} className="space-y-3">
              ...
              <button
                ...
                className="mt-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-500 to-blue-600 px-4 py-3 text-sm font-semibold text-white transition-all hover:from-indigo-600 hover:to-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {updatingPassword ? (
